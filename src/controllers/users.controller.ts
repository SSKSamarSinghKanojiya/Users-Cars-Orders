...
export class UsersController extends ResponseHelper {
  ...
  login:any = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (user) {
            ...
            if (passwordMatch) {
                ...
                httLogger.info(formatHTTPLoggerResponse(req,res,{user,token}))
      ...
      httLogger.error(formatHTTPLoggerResponse(req,res,{error:error.message}))
        console.log(error);
        throw error;
    }
}
